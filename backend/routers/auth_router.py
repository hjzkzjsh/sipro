...
from rbac import ALL_ROLES, effective_permissions, audit_log
from rbac_labels import resource_meta
...
async def session_payload(user: dict) -> dict:
    ...
    # Preferensi tampilan dari Pusat Konfigurasi (`ui.table_page_size`) — dipakai useListQuery.
    import settings_store as cfg
    u["ui"] = {"table_page_size": int(await cfg.get("ui.table_page_size",
                                                    org_id=user.get("org_id", ORG_ID)) or 25)}
    return u
...
async def update_me(payload: ProfileUpdate, user: dict = Depends(get_current_user)):
    """Profil sendiri: nama & telepon. Peran/email/status TIDAK bisa diubah dari sini (itu urusan admin)."""
    updates = {}
    ...
    await audit_log(user, "update", "profile", user["id"], {k: v for k, v in updates.items() if k != "updated_at"})
    fresh = await db.users.find_one({"id": user["id"]}, {"_id": 0})
    return {"data": await session_payload({**fresh, "org_id": user.get("org_id"), "active_org_id": user.get("active_org_id")})}
...
async def change_password(payload: PasswordChange, user: dict = Depends(get_current_user)):
    ...
    if payload.current_password == payload.new_password:
        raise HTTPException(400, "Kata sandi baru harus berbeda dari yang lama.")
    await db.users.update_one({"id": user["id"]}, {"$set": {"password_hash": hash_password(payload.new_password),
                                                           "updated_at": now_iso()}})
    await audit_log(user, "update", "profile", user["id"], {"password_changed": True})
    return {"data": {"ok": True}}


@router.get("/me/activity")
async def my_activity(user: dict = Depends(get_current_user)):
    """Jejak audit tindakan SAYA (20 terakhir) — pengguna berhak melihat apa yang tercatat atas namanya."""
    rows = await db.audit_logs.find({"actor": user.get("email")}, {"_id": 0}).sort("created_at", -1).to_list(20)
    return {"data": rows}

...
async def document_pdf(doc_id: str, user: dict = Depends(require_permission("documents", "view"))):
    ...
    money_rows = None
    if d.get("contract_id") and dl.section_visible(layout, "biaya"):
        import contracts_engine as ce
        import docgen as dg
        try:
            contract = await ce.get_raw(org, d["contract_id"])
            bd = await ce.build_breakdown(org, contract)
            money_rows = dl.money_rows_for(layout, dg.layout_amounts(bd)) or None
        except Exception:  # dokumen lama tanpa kontrak yang masih ada → tetap tercetak
            money_rows = None
